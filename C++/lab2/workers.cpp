...
#include "workers.h"
        ...
        const
    {
        ...
        if (previous.getType() != WorkerResult::ResultType::NONE)
    ...
    const WorkerResult WriteFile::execute(const WorkerResult& previous) const 
    {
        if (!isValidAcceptedResult(previous))
            return WorkerResult(WorkerResult::ResultType::UNKNOWN);
    ...
    static std::string replace(const std::string& str, const std::string& pattern, const std::string& substitution) 
    {
